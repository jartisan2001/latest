...
public class GitUtil {
	...
	/**
	   * 拉取远程仓库内容到本地
	 */
    public static void pullToLocal(String userName,String pwd,String gitRoot){
        //git仓库地址
    	log.info("git pull : {}",gitRoot);
    	try (FileRepository fileRepository =new FileRepository(gitRoot + File.separator + GIT)) {
	...
	public static boolean rollBackPreRevision(String gitRoot, List<DiffEntry> diffEntries, String revision,
		...
		try (Git git = Git.open(new File(gitRoot))) {
			...
			commitCmd.setCommitter("latest", "latest").setMessage(remark).call();
		}
		return true;
	}
